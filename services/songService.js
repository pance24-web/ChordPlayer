...
// Data awal sebagai in-memory fallback saat database offline atau belum dikonfigurasi
const defaultSongs = [
  {
    id: 1,
    title: 'Hapus Aku',
...
async function getAllSongs() {
  const rows = await dbQuery('SELECT id, title, artist FROM songs ORDER BY title ASC');
  if (rows && rows.length > 0) {
    return rows;
  }
  return defaultSongs.map(({ id, title, artist }) => ({ id, title, artist }));
}

// Ambil satu lagu berdasarkan ID (untuk halaman detail — termasuk chord lengkap)
async function getSongById(id) {
  const rows = await dbQuery('SELECT * FROM songs WHERE id = ?', [id]);
  if (rows && rows[0]) {
    return rows[0];
  }
  const song = defaultSongs.find(s => String(s.id) === String(id));
  return song || null;
}
